...
func (list StringList) Init() StringList {
	return StringList(list[:list.Len()-1])
}
...
func (list StringList) Reverse() StringList {
	numItems := list.Len()
	result := make(StringList, numItems)
	last := numItems - 1
	for i, v := range list {
		result[last-i] = v
	}
...
func (list StringList) TakeWhile(p func(*string) bool) (result StringList) {
	for _, v := range list {
		if p(v) {
			result = append(result, v)
		} else {
...
// elemense are added.
func (list StringList) DropWhile(p func(*string) bool) (result StringList) {
	adding := false
	for _, v := range list {
		if !p(v) || adding {
			adding = true
...
func (list StringList) Filter(fn func(*string) bool) StringList {
	result := make(StringList, 0, list.Len()/2)
...
func (list StringList) Partition(p func(*string) bool) (StringList, StringList) {
	matching := make(StringList, 0, list.Len()/2)
	others := make(StringList, 0, list.Len()/2)
...
func (list StringList) DistinctBy(equal func(*string, *string) bool) (result StringList) {
Outer:
	for _, v := range list {
