// A simple type derived from []{{.Type.Name}}
// Not thread-safe.
//
...
// {{.Prefix.U}}{{.Type.U}}List is a slice of type {{.Type.Name}}. Use it where you would use []{{.Type.Name}}.
// To add items to the list, simply use the normal built-in append function.
//
// List values follow a similar pattern to Scala Lists and LinearSeqs in particular.
// For comparison with Scala, see e.g. http://www.scala-lang.org/api/2.11.7/#scala.collection.LinearSeq
type {{.Prefix.U}}{{.Type.U}}List []{{.Type.Name}}
...
{{if and .Numeric (not .TypeIsPtr)}}
	for _, i := range values {
		switch i.(type) {
		case int:
			list = append(list, {{.Type.Name}}(i.(int)))
		case int8:
			list = append(list, {{.Type.Name}}(i.(int8)))
		case int16:
			list = append(list, {{.Type.Name}}(i.(int16)))
		case int32:
			list = append(list, {{.Type.Name}}(i.(int32)))
		case int64:
			list = append(list, {{.Type.Name}}(i.(int64)))
		case uint:
			list = append(list, {{.Type.Name}}(i.(uint)))
		case uint8:
			list = append(list, {{.Type.Name}}(i.(uint8)))
		case uint16:
			list = append(list, {{.Type.Name}}(i.(uint16)))
		case uint32:
			list = append(list, {{.Type.Name}}(i.(uint32)))
		case uint64:
			list = append(list, {{.Type.Name}}(i.(uint64)))
		case float32:
			list = append(list, {{.Type.Name}}(i.(float32)))
		case float64:
			list = append(list, {{.Type.Name}}(i.(float64)))
		}
	}
{{else}}
	for _, i := range values {
...
func (list {{.Prefix.U}}{{.Type.U}}List) Min() {{.Type.Name}} {
	m := list.MinBy(func(a {{.Type.Name}}, b {{.Type.Name}}) bool {
		return {{.Type.Star}}a < {{.Type.Star}}b
	})
	return {{.Type.Star}}m
}
...
func (list {{.Prefix.U}}{{.Type.U}}List) Max() (result {{.Type.Name}}) {
	m := list.MaxBy(func(a {{.Type.Name}}, b {{.Type.Name}}) bool {
		return {{.Type.Star}}a < {{.Type.Star}}b
	})
	return {{.Type.Star}}m
}
