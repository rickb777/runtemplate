...
{{if and .Numeric (not .TypeIsPtr)}}
	for _, i := range values {
		switch i.(type) {
		case int:
			set[{{.Type.Name}}(i.(int))] = struct{}{}
		case int8:
			set[{{.Type.Name}}(i.(int8))] = struct{}{}
		case int16:
			set[{{.Type.Name}}(i.(int16))] = struct{}{}
		case int32:
			set[{{.Type.Name}}(i.(int32))] = struct{}{}
		case int64:
			set[{{.Type.Name}}(i.(int64))] = struct{}{}
		case uint:
			set[{{.Type.Name}}(i.(uint))] = struct{}{}
		case uint8:
			set[{{.Type.Name}}(i.(uint8))] = struct{}{}
		case uint16:
			set[{{.Type.Name}}(i.(uint16))] = struct{}{}
		case uint32:
			set[{{.Type.Name}}(i.(uint32))] = struct{}{}
		case uint64:
			set[{{.Type.Name}}(i.(uint64))] = struct{}{}
		case float32:
			set[{{.Type.Name}}(i.(float32))] = struct{}{}
		case float64:
			set[{{.Type.Name}}(i.(float64))] = struct{}{}
		}
	}
{{else}}
	for _, i := range values {
...
func (set {{.Prefix.U}}{{.Type.U}}Set) Min() {{.Type.Name}} {
	return set.MinBy(func(a {{.Type.Name}}, b {{.Type.Name}}) bool {
		return a < b
	})
}
...
func (set {{.Prefix.U}}{{.Type.U}}Set) Max() (result {{.Type.Name}}) {
	return set.MaxBy(func(a {{.Type.Name}}, b {{.Type.Name}}) bool {
		return a < b
	})
}
{{- end}}
