...
// {{.UType}}Generator produces a stream of {{.PType}} based on a supplied generator function.
...
func {{.UType}}Generator(out chan<- {{.PType}}, iterations int, fn func(int) {{.PType}}) {
	{{.UType}}Generator3(out, 0, iterations-1, 1, fn)
}
...
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}Generator3(out chan<- {{.PType}}, from, to, stride int, fn func(int) {{.PType}}) {
...
// {{.UType}}Delta duplicates a stream of {{.PType}} to two output channels.
// When the sender closes the input channel, both output channels are closed then the function terminates.
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}Delta(in <-chan {{.PType}}, out1, out2 chan<- {{.PType}}) {
	for v := range in {
		select {
		case out1 <- v: out2 <- v
		case out2 <- v: out1 <- v
...
// {{.UType}}Zip2 interleaves two streams of {{.PType}}.
...
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}Zip2(in1, in2 <-chan {{.PType}}, out chan<- {{.PType}}) {
	closed2 := false
	for v := range in1 {
		out <- v
		v, ok := <- in2
...
// {{.UType}}Mux2 multiplexes two streams of {{.PType}} into a single output channel.
...
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}Mux2(in1, in2 <-chan {{.PType}}, closer <-chan bool, out chan<- {{.PType}}) {
	running := true
	for running {
		select {
		case v := <- in1:
	...
	go {{.UType}}BlackHole(in1)
	go {{.UType}}BlackHole(in2)
	close(out)
}

// {{.UType}}BlackHole silently consumes a stream of {{.PType}}.
// It terminates when the sender closes the channel.
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}BlackHole(in <-chan {{.PType}}) {
	for _ = range in {
		// om nom nom
	}
}

// {{.UType}}Filter filters a stream of {{.PType}}, silently dropping elements that do not match the predicate p.
// When the sender closes the input channel, the output channel is closed then the function terminates.
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}Filter(in <-chan {{.PType}}, out chan<- {{.PType}}, p func({{.PType}}) bool) {
	for v := range in {
		if p(v) {
			out <- v
		}
...
// {{.UType}}Partition filters a stream of {{.PType}} into two output streams using a predicate p, those that
// match and all others.
// When the sender closes the input channel, both output channels are closed then the function terminates.
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}Partition(in <-chan {{.PType}}, matching, others chan<- {{.PType}}, p func({{.PType}}) bool) {
	for v := range in {
		if p(v) {
			matching <- v
		} else {
...
// {{.UType}}Map transforms a stream of {{.PType}} by applying a function fn to each item in the stream.
// When the sender closes the input channel, the output channel is closed then the function terminates.
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}Map(in <-chan {{.PType}}, out chan<- {{.PType}}, fn func({{.PType}}) {{.PType}}) {
	for v := range in {
		out <- fn(v)
	}
	close(out)
...
// {{.UType}}FlatMap transforms a stream of {{.PType}} by applying a function fn to each item in the stream that
// gives zero or more results, all of which are sent out.
// When the sender closes the input channel, the output channel is closed then the function terminates.
//
// It is part of the Plumbing function suite for {{.UType}}.
func {{.UType}}FlatMap(in <-chan {{.PType}}, out chan<- {{.PType}}, fn func({{.PType}}) {{.UType}}Collection) {
	for vi := range in {
		c := fn(vi)
		if c.NonEmpty() {
			for vo := range c.Send() {
