...
// PAppleGenerator produces a stream of *Apple based on a supplied generator function.
...
func PAppleGenerator(out chan<- *Apple, iterations int, fn func(int) *Apple) {
	PAppleGenerator3(out, 0, iterations-1, 1, fn)
}
...
//
// It is part of the Plumbing function suite for Apple.
func PAppleGenerator3(out chan<- *Apple, from, to, stride int, fn func(int) *Apple) {
...
// PAppleDelta duplicates a stream of *Apple to two output channels.
// When the sender closes the input channel, both output channels are closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func PAppleDelta(in <-chan *Apple, out1, out2 chan<- *Apple) {
	for v := range in {
		select {
		case out1 <- v: out2 <- v
		case out2 <- v: out1 <- v
...
// PAppleZip2 interleaves two streams of *Apple.
...
//
// It is part of the Plumbing function suite for Apple.
func PAppleZip2(in1, in2 <-chan *Apple, out chan<- *Apple) {
	closed2 := false
	for v := range in1 {
		out <- v
		v, ok := <- in2
...
// PAppleMux2 multiplexes two streams of *Apple into a single output channel.
...
//
// It is part of the Plumbing function suite for Apple.
func PAppleMux2(in1, in2 <-chan *Apple, closer <-chan bool, out chan<- *Apple) {
	running := true
	for running {
		select {
		case v := <- in1:
	...
	go PAppleBlackHole(in1)
	go PAppleBlackHole(in2)
	close(out)
}

// PAppleBlackHole silently consumes a stream of *Apple.
// It terminates when the sender closes the channel.
//
// It is part of the Plumbing function suite for Apple.
func PAppleBlackHole(in <-chan *Apple) {
	for _ = range in {
		// om nom nom
	}
}

// PAppleFilter filters a stream of *Apple, silently dropping elements that do not match the predicate p.
// When the sender closes the input channel, the output channel is closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func PAppleFilter(in <-chan *Apple, out chan<- *Apple, p func(*Apple) bool) {
	for v := range in {
		if p(v) {
			out <- v
		}
...
// PApplePartition filters a stream of *Apple into two output streams using a predicate p, those that
// match and all others.
// When the sender closes the input channel, both output channels are closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func PApplePartition(in <-chan *Apple, matching, others chan<- *Apple, p func(*Apple) bool) {
	for v := range in {
		if p(v) {
			matching <- v
		} else {
...
// PAppleMap transforms a stream of *Apple by applying a function fn to each item in the stream.
// When the sender closes the input channel, the output channel is closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func PAppleMap(in <-chan *Apple, out chan<- *Apple, fn func(*Apple) *Apple) {
	for v := range in {
		out <- fn(v)
	}
	close(out)
...
// PAppleFlatMap transforms a stream of *Apple by applying a function fn to each item in the stream that
