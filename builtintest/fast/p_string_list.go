...
func (list PStringList) Reverse() PStringList {
	numItems := list.Len()
	result := make(PStringList, numItems)
	last := numItems - 1
	for i, v := range list {
		result[last-i] = v
	}
...
func (list PStringList) TakeWhile(p func(*string) bool) (result PStringList) {
	for _, v := range list {
		if p(v) {
			result = append(result, v)
		} else {
...
// elemense are added.
func (list PStringList) DropWhile(p func(*string) bool) (result PStringList) {
	adding := false
	for _, v := range list {
		if !p(v) || adding {
			adding = true
...
func (list PStringList) Filter(fn func(*string) bool) PStringList {
	result := make(PStringList, 0, list.Len()/2)
...
func (list PStringList) Partition(p func(*string) bool) (PStringList, PStringList) {
	matching := make(PStringList, 0, list.Len()/2)
	others := make(PStringList, 0, list.Len()/2)
...
func (list PStringList) DistinctBy(equal func(*string, *string) bool) (result PStringList) {
Outer:
	for _, v := range list {
