...
func (list XStringList) Reverse() XStringList {
	numItems := list.Len()
	result := make(XStringList, numItems)
	last := numItems - 1
	for i, v := range list {
		result[last-i] = v
	}
...
func (list XStringList) TakeWhile(p func(string) bool) (result XStringList) {
	for _, v := range list {
		if p(v) {
			result = append(result, v)
		} else {
...
// elemense are added.
func (list XStringList) DropWhile(p func(string) bool) (result XStringList) {
	adding := false
	for _, v := range list {
		if !p(v) || adding {
			adding = true
...
func (list XStringList) Filter(fn func(string) bool) XStringList {
	result := make(XStringList, 0, list.Len()/2)
...
func (list XStringList) Partition(p func(string) bool) (XStringList, XStringList) {
	matching := make(XStringList, 0, list.Len()/2)
	others := make(XStringList, 0, list.Len()/2)
...
func (list XStringList) DistinctBy(equal func(string, string) bool) (result XStringList) {
Outer:
	for _, v := range list {
