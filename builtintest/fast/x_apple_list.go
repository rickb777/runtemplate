...
func (list XAppleList) Reverse() XAppleList {
	numItems := list.Len()
	result := make(XAppleList, numItems)
	last := numItems - 1
	for i, v := range list {
		result[last-i] = v
	}
...
func (list XAppleList) TakeWhile(p func(Apple) bool) (result XAppleList) {
	for _, v := range list {
		if p(v) {
			result = append(result, v)
		} else {
...
// elemense are added.
func (list XAppleList) DropWhile(p func(Apple) bool) (result XAppleList) {
	adding := false
	for _, v := range list {
		if !p(v) || adding {
			adding = true
...
func (list XAppleList) Filter(fn func(Apple) bool) XAppleList {
	result := make(XAppleList, 0, list.Len()/2)
...
func (list XAppleList) Partition(p func(Apple) bool) (XAppleList, XAppleList) {
	matching := make(XAppleList, 0, list.Len()/2)
	others := make(XAppleList, 0, list.Len()/2)
...
func (list XAppleList) DistinctBy(equal func(Apple, Apple) bool) (result XAppleList) {
Outer:
	for _, v := range list {
