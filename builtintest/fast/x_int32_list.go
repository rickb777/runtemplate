...
func (list XInt32List) Reverse() XInt32List {
	numItems := list.Len()
	result := make(XInt32List, numItems)
	last := numItems - 1
	for i, v := range list {
		result[last-i] = v
	}
...
func (list XInt32List) TakeWhile(p func(int32) bool) (result XInt32List) {
	for _, v := range list {
		if p(v) {
			result = append(result, v)
		} else {
...
// elemense are added.
func (list XInt32List) DropWhile(p func(int32) bool) (result XInt32List) {
	adding := false
	for _, v := range list {
		if !p(v) || adding {
			adding = true
...
func (list XInt32List) Filter(fn func(int32) bool) XInt32List {
	result := make(XInt32List, 0, list.Len()/2)
...
func (list XInt32List) Partition(p func(int32) bool) (XInt32List, XInt32List) {
	matching := make(XInt32List, 0, list.Len()/2)
	others := make(XInt32List, 0, list.Len()/2)
...
func (list XInt32List) DistinctBy(equal func(int32, int32) bool) (result XInt32List) {
Outer:
	for _, v := range list {
...
// Panics if the collection is empty.
func (list XInt32List) Max() int32 {
