...
func (mm SP1AppleStringMap) Pop(k *Apple) (*string, bool) {
	v, found := mm[k]
	delete(mm, k)
	return v, found
}
...
func (mm SP1AppleStringMap) DropWhere(fn func(*Apple, *string) bool) SP1AppleStringTuples {
	removed := make(SP1AppleStringTuples, 0)
	for k, v := range mm {
		if fn(k, v) {
			removed = append(removed, SP1AppleStringTuple{k, v})
			delete(mm, k)
		}
	}
	return removed
...
func (mm SP1AppleStringMap) Foreach(fn func(*Apple, *string)) {
	for k, v := range mm {
		fn(k, v)
	}
}
...
func (mm SP1AppleStringMap) Find(fn func(*Apple, *string) bool) (SP1AppleStringTuple, bool) {
	for k, v := range mm {
		if fn(k, v) {
			return SP1AppleStringTuple{k, v}, true
		}
	}

	return SP1AppleStringTuple{}, false
}
...
func (mm SP1AppleStringMap) Map(fn func(*Apple, *string) (*Apple, *string)) SP1AppleStringMap {
	result := NewSP1AppleStringMap()
...
// This is a domain-to-range mapping function. For bespoke transformations to other types, copy and modify
// this method appropriately.
func (mm SP1AppleStringMap) FlatMap(fn func(*Apple, *string) []SP1AppleStringTuple) SP1AppleStringMap {
	result := NewSP1AppleStringMap()
