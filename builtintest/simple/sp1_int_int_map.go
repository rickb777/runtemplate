...
func (mm SP1IntIntMap) Pop(k *int) (*int, bool) {
	v, found := mm[k]
	delete(mm, k)
	return v, found
}
...
func (mm SP1IntIntMap) DropWhere(fn func(*int, *int) bool) SP1IntIntTuples {
	removed := make(SP1IntIntTuples, 0)
	for k, v := range mm {
		if fn(k, v) {
			removed = append(removed, SP1IntIntTuple{k, v})
			delete(mm, k)
		}
	}
	return removed
...
func (mm SP1IntIntMap) Foreach(fn func(*int, *int)) {
	for k, v := range mm {
		fn(k, v)
	}
}
...
func (mm SP1IntIntMap) Find(fn func(*int, *int) bool) (SP1IntIntTuple, bool) {
	for k, v := range mm {
		if fn(k, v) {
			return SP1IntIntTuple{k, v}, true
		}
	}

	return SP1IntIntTuple{}, false
}
...
func (mm SP1IntIntMap) Map(fn func(*int, *int) (*int, *int)) SP1IntIntMap {
	result := NewSP1IntIntMap()
...
// This is a domain-to-range mapping function. For bespoke transformations to other types, copy and modify
// this method appropriately.
func (mm SP1IntIntMap) FlatMap(fn func(*int, *int) []SP1IntIntTuple) SP1IntIntMap {
	result := NewSP1IntIntMap()
