...
func (mm SP1StringStringMap) Pop(k *string) (*string, bool) {
	v, found := mm[k]
	delete(mm, k)
	return v, found
}
...
func (mm SP1StringStringMap) DropWhere(fn func(*string, *string) bool) SP1StringStringTuples {
	removed := make(SP1StringStringTuples, 0)
	for k, v := range mm {
		if fn(k, v) {
			removed = append(removed, SP1StringStringTuple{k, v})
			delete(mm, k)
		}
	}
	return removed
...
func (mm SP1StringStringMap) Foreach(fn func(*string, *string)) {
	for k, v := range mm {
		fn(k, v)
	}
}
...
func (mm SP1StringStringMap) Find(fn func(*string, *string) bool) (SP1StringStringTuple, bool) {
	for k, v := range mm {
		if fn(k, v) {
			return SP1StringStringTuple{k, v}, true
		}
	}

	return SP1StringStringTuple{}, false
}
