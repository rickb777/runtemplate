...
func (mm SP1StringIntMap) Pop(k *string) (*big.Int, bool) {
	v, found := mm[k]
	delete(mm, k)
	return v, found
}
...
func (mm SP1StringIntMap) DropWhere(fn func(*string, *big.Int) bool) SP1StringIntTuples {
	removed := make(SP1StringIntTuples, 0)
	for k, v := range mm {
		if fn(k, v) {
			removed = append(removed, SP1StringIntTuple{k, v})
			delete(mm, k)
		}
	}
	return removed
...
func (mm SP1StringIntMap) Foreach(fn func(*string, *big.Int)) {
	for k, v := range mm {
		fn(k, v)
	}
}
...
func (mm SP1StringIntMap) Find(fn func(*string, *big.Int) bool) (SP1StringIntTuple, bool) {
	for k, v := range mm {
		if fn(k, v) {
			return SP1StringIntTuple{k, v}, true
		}
	}

	return SP1StringIntTuple{}, false
}
...
// Map returns a new SP1IntMap by transforming every element with a function fn.
// The original map is not modified.
//
// This is a domain-to-range mapping function. For bespoke transformations to other types, copy and modify
// this method appropriately.
func (mm SP1StringIntMap) Map(fn func(*string, *big.Int) (*string, *big.Int)) SP1StringIntMap {
	result := NewSP1StringIntMap()
...
// FlatMap returns a new SP1IntMap by transforming every element with a function fn that
// returns zero or more items in a slice. The resulting map may have a different size to the original map.
// The original map is not modified.
//
// This is a domain-to-range mapping function. For bespoke transformations to other types, copy and modify
// this method appropriately.
func (mm SP1StringIntMap) FlatMap(fn func(*string, *big.Int) []SP1StringIntTuple) SP1StringIntMap {
	result := NewSP1StringIntMap()
