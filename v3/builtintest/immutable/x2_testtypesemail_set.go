// An encapsulated immutable map[testtypes.Email]struct{} used as a set.
// Thread-safe.
//
//
...
func (set *X2TesttypesEmailSet) Union(other *X2TesttypesEmailSet) *X2TesttypesEmailSet {
	...
	unionedSet := NewX2TesttypesEmailSet()
...
func (set *X2TesttypesEmailSet) Remove(i testtypes.Email) *X2TesttypesEmailSet {
	if set == nil {
		return nil
	}
...
// Foreach iterates over testtypes.EmailSet and executes the function f against each element.
...
// Partition returns two new testtypes.EmailSets whose elements return true or false for the predicate, p.
