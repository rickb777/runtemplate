...
// P1AppleQueue is a ring buffer containing a slice of type *Apple. It is optimised
// for FIFO operations.
type P1AppleQueue struct {
	m         []*Apple
	read      int
	write     int
	length    int
...
// NewP1AppleQueue returns a new queue of *Apple. The behaviour when adding
...
func NewP1AppleQueue(capacity int, overwrite bool) *P1AppleQueue {
	return NewP1AppleSortedQueue(capacity, overwrite, nil)
}

// NewP1AppleSortedQueue returns a new queue of *Apple. The behaviour when adding
// to the queue depends on overwrite. If true, the push operation overwrites oldest values up to
// the space available, when the queue is full. Otherwise, it refuses to overfill the queue.
// If the 'less' comparison function is not nil, elements can be easily sorted.
func NewP1AppleSortedQueue(capacity int, overwrite bool, less func(i, j *Apple) bool) *P1AppleQueue {
	return &P1AppleQueue{
...
// BuildP1AppleQueueFromChan constructs a new P1AppleQueue from a channel that supplies
// a sequence of values until it is closed. The function doesn't return until then.
func BuildP1AppleQueueFromChan(source <-chan *Apple) *P1AppleQueue {
	queue := NewP1AppleQueue(0, false)
	for v := range source {
...
func (queue *P1AppleQueue) Reallocate(capacity int, overwrite bool) *P1AppleQueue {
	if capacity < 1 {
...
func (queue *P1AppleQueue) doReallocate(capacity int, overwrite bool) *P1AppleQueue {
...
func (queue *P1AppleQueue) Space() int {
	if queue == nil {
		return 0
	}
	queue.s.RLock()
...
func (queue *P1AppleQueue) Cap() int {
	if queue == nil {
		return 0
	}
...
func (queue *P1AppleQueue) ToList() *P1AppleList {
	...
	list := MakeP1AppleList(queue.length, queue.length)
...
// copy; the queue is not altered.
func (queue *P1AppleQueue) ToSet() *P1AppleSet {
	if queue == nil {
		return nil
	}
...
func (queue *P1AppleQueue) toSlice(s []*Apple) []*Apple {
...
func (queue *P1AppleQueue) ToInterfaceSlice() []interface{} {
	if queue == nil {
		return nil
	}
...
func (queue *P1AppleQueue) Clone() *P1AppleQueue {
	if queue == nil {
		return nil
	}
...
func (queue *P1AppleQueue) Get(i int) *Apple {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
func (queue *P1AppleQueue) Head() *Apple {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
func (queue *P1AppleQueue) HeadOption() (*Apple, bool) {
	if queue == nil {
		return nil, false
	}
...
// Panics if queue is empty or nil.
func (queue *P1AppleQueue) Last() *Apple {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
func (queue *P1AppleQueue) LastOption() (*Apple, bool) {
	if queue == nil {
		return nil, false
	}
...
func (queue *P1AppleQueue) IsOverwriting() bool {
	if queue == nil {
		return false
	}
	queue.s.RLock()
...
func (queue *P1AppleQueue) IsFull() bool {
	if queue == nil {
		return false
	}
	queue.s.RLock()
...
func (queue *P1AppleQueue) IsEmpty() bool {
	if queue == nil {
		return true
	}
	queue.s.RLock()
...
func (queue *P1AppleQueue) NonEmpty() bool {
	if queue == nil {
		return false
	}
	queue.s.RLock()
...
func (queue *P1AppleQueue) Size() int {
	if queue == nil {
		return 0
	}
	queue.s.RLock()
...
func (queue *P1AppleQueue) Len() int {
	return queue.Size()
}

// Swap swaps the elements with indexes i and j.
// The queue must not be empty.
func (queue *P1AppleQueue) Swap(i, j int) {
	ri := (queue.read + i) % queue.capacity
...
// be empty.
func (queue *P1AppleQueue) Less(i, j int) bool {
	ri := (queue.read + i) % queue.capacity
	rj := (queue.read + j) % queue.capacity
...
// (see NewP1AppleSortedQueue).
func (queue *P1AppleQueue) Sort() {
	sort.Sort(queue)
}
...
// This function will panic if the collection was created with a nil 'less' function
// (see NewP1AppleSortedQueue).
func (queue *P1AppleQueue) StableSort() {
	sort.Stable(queue)
}
...
func (queue *P1AppleQueue) frontAndBack() ([]*Apple, []*Apple) {
...
func (queue *P1AppleQueue) indexes() []int {
	if queue == nil || queue.length == 0 {
		return nil
	}
	if queue.write > queue.read {
...
func (queue *P1AppleQueue) Add(more ...*Apple) {
	queue.Push(more...)
}
...
func (queue *P1AppleQueue) Push(items ...*Apple) *P1AppleQueue {
	queue.s.Lock()
	defer queue.s.Unlock()
...
func (queue *P1AppleQueue) Offer(items ...*Apple) []*Apple {
	queue.s.Lock()
	defer queue.s.Unlock()
	return queue.doPush(items...)
}

func (queue *P1AppleQueue) doPush(items ...*Apple) []*Apple {
	n := len(items)
...
func (queue *P1AppleQueue) Pop1() (*Apple, bool) {
	queue.s.Lock()
	defer queue.s.Unlock()
...
func (queue *P1AppleQueue) Pop(n int) []*Apple {
	queue.s.Lock()
	defer queue.s.Unlock()
	return queue.doPop(n)
}

func (queue *P1AppleQueue) doPop(n int) []*Apple {
	if queue.length == 0 {
		return nil
	}
...
func (queue *P1AppleQueue) Contains(v *Apple) bool {
...
func (queue *P1AppleQueue) ContainsAll(i ...*Apple) bool {
	if queue == nil {
		return len(i) == 0
	}
...
// Exists verifies that one or more elements of P1AppleQueue return true for the predicate p.
// The function should not alter the values via side-effects.
func (queue *P1AppleQueue) Exists(p func(*Apple) bool) bool {
	if queue == nil {
		return false
	}
...
// Forall verifies that all elements of P1AppleQueue return true for the predicate p.
// The function should not alter the values via side-effects.
func (queue *P1AppleQueue) Forall(p func(*Apple) bool) bool {
	if queue == nil {
		return true
	}
...
// Foreach iterates over P1AppleQueue and executes function f against each element.
// The function can safely alter the values via side-effects.
func (queue *P1AppleQueue) Foreach(f func(*Apple)) {
	if queue == nil {
		return
	}
...
func (queue *P1AppleQueue) Send() <-chan *Apple {
	ch := make(chan *Apple)
	go func() {
		if queue != nil {
			queue.s.RLock()
...
// DoKeepWhere modifies a P1AppleQueue by retaining only those elements that match
...
func (queue *P1AppleQueue) DoKeepWhere(p func(*Apple) bool) *P1AppleQueue {
	if queue == nil {
		return nil
	}
...
func (queue *P1AppleQueue) Find(p func(*Apple) bool) (*Apple, bool) {
	if queue == nil {
		return nil, false
	}
...
// Filter returns a new P1AppleQueue whose elements return true for predicate p.
//
// The original queue is not modified. See also DoKeepWhere (which does modify the original queue).
func (queue *P1AppleQueue) Filter(p func(*Apple) bool) *P1AppleQueue {
	if queue == nil {
		return nil
	}
	...
	result := NewP1AppleSortedQueue(queue.length, queue.overwrite, queue.less)
	i := 0
...
// Partition returns two new P1AppleQueues whose elements return true or false for the predicate, p.
// The first result consists of all elements that satisfy the predicate and the second result consists of
// all elements that don't. The relative order of the elements in the results is the same as in the
// original queue.
//
// The original queue is not modified
func (queue *P1AppleQueue) Partition(p func(*Apple) bool) (*P1AppleQueue, *P1AppleQueue) {
	if queue == nil {
		return nil, nil
	}
	...
	matching := NewP1AppleSortedQueue(queue.length, queue.overwrite, queue.less)
	others := NewP1AppleSortedQueue(queue.length, queue.overwrite, queue.less)
	m, o := 0, 0
...
// Map returns a new P1AppleQueue by transforming every element with function f.
// The resulting queue is the same size as the original queue.
// The original queue is not modified.
//
// This is a domain-to-range mapping function. For bespoke transformations to other types, copy and modify
// this method appropriately.
func (queue *P1AppleQueue) Map(f func(*Apple) *Apple) *P1AppleQueue {
	if queue == nil {
		return nil
	}
...
// FlatMap returns a new P1AppleQueue by transforming every element with function f that
// returns zero or more items in a slice. The resulting queue may have a different size to the original queue.
// The original queue is not modified.
//
// This is a domain-to-range mapping function. For bespoke transformations to other types, copy and modify
// this method appropriately.
func (queue *P1AppleQueue) FlatMap(f func(*Apple) []*Apple) *P1AppleQueue {
	if queue == nil {
		return nil
	}
...
// CountBy gives the number elements of P1AppleQueue that return true for the predicate p.
func (queue *P1AppleQueue) CountBy(p func(*Apple) bool) (result int) {
	if queue == nil {
		return 0
	}
...
func (queue *P1AppleQueue) Fold(initial *Apple, fn func(*Apple, *Apple) *Apple) *Apple {
	if queue == nil {
		return initial
	}
...
// MinBy returns an element of P1AppleQueue containing the minimum value, when compared to other elements
// using a passed func defining ‘less’. In the case of multiple items being equally minimal, the first such
// element is returned. Panics if there are no elements.
func (queue *P1AppleQueue) MinBy(less func(*Apple, *Apple) bool) *Apple {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
// MaxBy returns an element of P1AppleQueue containing the maximum value, when compared to other elements
// using a passed func defining ‘less’. In the case of multiple items being equally maximal, the first such
// element is returned. Panics if there are no elements.
func (queue *P1AppleQueue) MaxBy(less func(*Apple, *Apple) bool) *Apple {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
func (queue *P1AppleQueue) Equals(other *P1AppleQueue) bool {
	if queue == nil {
		if other == nil {
			return true
		}
