...
// P1AppleGenerator produces a stream of *Apple based on a supplied generator function.
...
func P1AppleGenerator(out chan<- *Apple, iterations int, fn func(int) *Apple) {
	P1AppleGenerator3(out, 0, iterations-1, 1, fn)
}
...
//
// It is part of the Plumbing function suite for Apple.
func P1AppleGenerator3(out chan<- *Apple, from, to, stride int, fn func(int) *Apple) {
...
// P1AppleDelta duplicates a stream of *Apple to two output channels.
// When the sender closes the input channel, both output channels are closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func P1AppleDelta(in <-chan *Apple, out1, out2 chan<- *Apple) {
	for v := range in {
		select {
		case out1 <- v:
			out2 <- v
...
// P1AppleZip2 interleaves two streams of *Apple.
...
//
// It is part of the Plumbing function suite for Apple.
func P1AppleZip2(in1, in2 <-chan *Apple, out chan<- *Apple) {
	closed2 := false
	for v := range in1 {
		out <- v
		v, ok := <- in2
...
// P1AppleMux2 multiplexes two streams of *Apple into a single output channel.
...
//
// It is part of the Plumbing function suite for Apple.
func P1AppleMux2(in1, in2 <-chan *Apple, closer <-chan bool, out chan<- *Apple) {
	running := true
	for running {
		select {
		case v := <- in1:
	...
	go P1AppleBlackHole(in1)
	go P1AppleBlackHole(in2)
	close(out)
}

// P1AppleBlackHole silently consumes a stream of *Apple.
// It terminates when the sender closes the channel.
//
// It is part of the Plumbing function suite for Apple.
func P1AppleBlackHole(in <-chan *Apple) {
	for _ = range in {
		// om nom nom
	}
}

// P1AppleFilter filters a stream of *Apple, silently dropping elements that do not match the predicate p.
// When the sender closes the input channel, the output channel is closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func P1AppleFilter(in <-chan *Apple, out chan<- *Apple, p func(*Apple) bool) {
	for v := range in {
		if p(v) {
			out <- v
		}
...
// P1ApplePartition filters a stream of *Apple into two output streams using a predicate p, those that
// match and all others.
// When the sender closes the input channel, both output channels are closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func P1ApplePartition(in <-chan *Apple, matching, others chan<- *Apple, p func(*Apple) bool) {
	for v := range in {
		if p(v) {
			matching <- v
		} else {
...
// P1AppleMap transforms a stream of *Apple by applying a function fn to each item in the stream.
// When the sender closes the input channel, the output channel is closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func P1AppleMap(in <-chan *Apple, out chan<- *Apple, fn func(*Apple) *Apple) {
	for v := range in {
		out <- fn(v)
	}
	close(out)
...
// P1AppleFlatMap transforms a stream of *Apple by applying a function fn to each item in the stream that
// gives zero or more results, all of which are sent out.
// When the sender closes the input channel, the output channel is closed then the function terminates.
//
// It is part of the Plumbing function suite for Apple.
func P1AppleFlatMap(in <-chan *Apple, out chan<- *Apple, fn func(*Apple) P1AppleCollection) {
	for vi := range in {
		c := fn(vi)
