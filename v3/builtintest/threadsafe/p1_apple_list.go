...
// P1AppleList contains a slice of type *Apple.
...
func (list *P1AppleList) Append(more ...*Apple) *P1AppleList {
	if list == nil {
		...
		list = MakeP1AppleList(0, len(more))
	}
...
func (list *P1AppleList) Filter(p func(*Apple) bool) *P1AppleList {
	...
	result := MakeP1AppleList(0, len(list.m))
...
// Partition returns two new P1AppleLists whose elements return true or false for the predicate, p.
...
func (list *P1AppleList) Partition(p func(*Apple) bool) (*P1AppleList, *P1AppleList) {
	...
	matching := MakeP1AppleList(0, len(list.m))
	others := MakeP1AppleList(0, len(list.m))
...
func (list *P1AppleList) Map(f func(*Apple) *Apple) *P1AppleList {
	...
	result := MakeP1AppleList(len(list.m), len(list.m))
	list.s.RLock()
	defer list.s.RUnlock()
...
func (list *P1AppleList) Fold(initial *Apple, fn func(*Apple, *Apple) *Apple) *Apple {
	list.s.RLock()
	defer list.s.RUnlock()
