...
// P1StringQueue is a ring buffer containing a slice of type *string. It is optimised
// for FIFO operations.
type P1StringQueue struct {
	m         []*string
	read      int
	write     int
	length    int
...
// NewP1StringQueue returns a new queue of *string. The behaviour when adding
...
func NewP1StringQueue(capacity int, overwrite bool) *P1StringQueue {
	return NewP1StringSortedQueue(capacity, overwrite, nil)
}

// NewP1StringSortedQueue returns a new queue of *string. The behaviour when adding
// to the queue depends on overwrite. If true, the push operation overwrites oldest values up to
// the space available, when the queue is full. Otherwise, it refuses to overfill the queue.
// If the 'less' comparison function is not nil, elements can be easily sorted.
func NewP1StringSortedQueue(capacity int, overwrite bool, less func(i, j *string) bool) *P1StringQueue {
...
// BuildP1StringQueueFromChan constructs a new P1StringQueue from a channel that supplies
// a sequence of values until it is closed. The function doesn't return until then.
func BuildP1StringQueueFromChan(source <-chan *string) *P1StringQueue {
	queue := NewP1StringQueue(0, false)
	for v := range source {
...
func (queue *P1StringQueue) Reallocate(capacity int, overwrite bool) *P1StringQueue {
	if capacity < 1 {
...
func (queue *P1StringQueue) doReallocate(capacity int, overwrite bool) *P1StringQueue {
...
func (queue *P1StringQueue) Space() int {
	if queue == nil {
		return 0
	}
	queue.s.RLock()
...
func (queue *P1StringQueue) Cap() int {
	if queue == nil {
		return 0
	}
...
func (queue *P1StringQueue) IsSequence() bool {
	return true
}

// IsSet returns false for lists or queues.
func (queue *P1StringQueue) IsSet() bool {
	return false
}

// ToList returns the elements of the queue as a list. The returned list is a shallow
// copy; the queue is not altered.
func (queue *P1StringQueue) ToList() *P1StringList {
	if queue == nil {
		return nil
	}
	...
	list := MakeP1StringList(queue.length, queue.length)
...
func (queue *P1StringQueue) toSlice(s []*string) []*string {
...
func (queue *P1StringQueue) ToInterfaceSlice() []interface{} {
	if queue == nil {
		return nil
	}
...
func (queue *P1StringQueue) Clone() *P1StringQueue {
	if queue == nil {
		return nil
	}
...
func (queue *P1StringQueue) Get(i int) *string {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
func (queue *P1StringQueue) Head() *string {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
// Panics if queue is empty or nil.
func (queue *P1StringQueue) Last() *string {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
func (queue *P1StringQueue) IsOverwriting() bool {
	if queue == nil {
		return false
	}
	queue.s.RLock()
...
func (queue *P1StringQueue) IsFull() bool {
	if queue == nil {
		return false
	}
	queue.s.RLock()
...
func (queue *P1StringQueue) IsEmpty() bool {
	if queue == nil {
		return true
	}
	queue.s.RLock()
...
func (queue *P1StringQueue) NonEmpty() bool {
	if queue == nil {
		return false
	}
	queue.s.RLock()
...
func (queue *P1StringQueue) Size() int {
	if queue == nil {
		return 0
	}
	queue.s.RLock()
...
func (queue *P1StringQueue) Len() int {
	return queue.Size()
}

// Swap swaps the elements with indexes i and j.
// The queue must not be empty.
func (queue *P1StringQueue) Swap(i, j int) {
	ri := (queue.read + i) % queue.capacity
...
// be empty.
func (queue *P1StringQueue) Less(i, j int) bool {
	ri := (queue.read + i) % queue.capacity
	rj := (queue.read + j) % queue.capacity
...
// (see NewP1StringSortedQueue).
func (queue *P1StringQueue) Sort() {
	sort.Sort(queue)
}
...
// This function will panic if the collection was created with a nil 'less' function
// (see NewP1StringSortedQueue).
func (queue *P1StringQueue) StableSort() {
	sort.Stable(queue)
}
...
func (queue *P1StringQueue) frontAndBack() ([]*string, []*string) {
...
func (queue *P1StringQueue) indexes() []int {
	if queue == nil || queue.length == 0 {
		return nil
	}
	if queue.write > queue.read {
...
func (queue *P1StringQueue) Add(more ...*string) {
	queue.Push(more...)
}
...
func (queue *P1StringQueue) Push(items ...*string) *P1StringQueue {
	queue.s.Lock()
	defer queue.s.Unlock()
...
func (queue *P1StringQueue) Offer(items ...*string) []*string {
	queue.s.Lock()
	defer queue.s.Unlock()
	return queue.doPush(items...)
}

func (queue *P1StringQueue) doPush(items ...*string) []*string {
	n := len(items)
...
func (queue *P1StringQueue) Pop1() (*string, bool) {
	queue.s.Lock()
	defer queue.s.Unlock()
...
func (queue *P1StringQueue) Pop(n int) []*string {
	queue.s.Lock()
	defer queue.s.Unlock()
	return queue.doPop(n)
}

func (queue *P1StringQueue) doPop(n int) []*string {
	if queue.length == 0 {
		return nil
	}
...
func (queue *P1StringQueue) Contains(v *string) bool {
...
func (queue *P1StringQueue) ContainsAll(i ...*string) bool {
	if queue == nil {
		return len(i) == 0
	}
...
// Exists verifies that one or more elements of P1StringQueue return true for the predicate p.
// The function should not alter the values via side-effects.
func (queue *P1StringQueue) Exists(p func(*string) bool) bool {
	if queue == nil {
		return false
	}
...
// Forall verifies that all elements of P1StringQueue return true for the predicate p.
// The function should not alter the values via side-effects.
func (queue *P1StringQueue) Forall(p func(*string) bool) bool {
	if queue == nil {
		return true
	}
...
// Foreach iterates over P1StringQueue and executes function f against each element.
// The function can safely alter the values via side-effects.
func (queue *P1StringQueue) Foreach(f func(*string)) {
	if queue == nil {
		return
	}
...
func (queue *P1StringQueue) Send() <-chan *string {
	ch := make(chan *string)
	go func() {
		if queue != nil {
			queue.s.RLock()
...
// DoKeepWhere modifies a P1StringQueue by retaining only those elements that match
...
func (queue *P1StringQueue) DoKeepWhere(p func(*string) bool) *P1StringQueue {
	if queue == nil {
		return nil
	}
...
func (queue *P1StringQueue) Find(p func(*string) bool) (*string, bool) {
	if queue == nil {
		return nil, false
	}
...
// Filter returns a new P1StringQueue whose elements return true for predicate p.
//
// The original queue is not modified. See also DoKeepWhere (which does modify the original queue).
func (queue *P1StringQueue) Filter(p func(*string) bool) *P1StringQueue {
	if queue == nil {
		return nil
	}
	...
	result := NewP1StringSortedQueue(queue.length, queue.overwrite, queue.less)
	i := 0
...
// Partition returns two new P1StringQueues whose elements return true or false for the predicate, p.
// The first result consists of all elements that satisfy the predicate and the second result consists of
// all elements that don't. The relative order of the elements in the results is the same as in the
// original queue.
//
// The original queue is not modified
func (queue *P1StringQueue) Partition(p func(*string) bool) (*P1StringQueue, *P1StringQueue) {
	if queue == nil {
		return nil, nil
	}
	...
	matching := NewP1StringSortedQueue(queue.length, queue.overwrite, queue.less)
	others := NewP1StringSortedQueue(queue.length, queue.overwrite, queue.less)
	m, o := 0, 0
...
// Map returns a new P1StringQueue by transforming every element with function f.
// The resulting queue is the same size as the original queue.
// The original queue is not modified.
//
// This is a domain-to-range mapping function. For bespoke transformations to other types, copy and modify
// this method appropriately.
func (queue *P1StringQueue) Map(f func(*string) *string) *P1StringQueue {
	if queue == nil {
		return nil
	}
...
// FlatMap returns a new P1StringQueue by transforming every element with function f that
// returns zero or more items in a slice. The resulting queue may have a different size to the original queue.
// The original queue is not modified.
//
// This is a domain-to-range mapping function. For bespoke transformations to other types, copy and modify
// this method appropriately.
func (queue *P1StringQueue) FlatMap(f func(*string) []*string) *P1StringQueue {
	if queue == nil {
		return nil
	}
...
// CountBy gives the number elements of P1StringQueue that return true for the predicate p.
func (queue *P1StringQueue) CountBy(p func(*string) bool) (result int) {
	if queue == nil {
		return 0
	}
...
func (queue *P1StringQueue) Fold(initial *string, fn func(*string, *string) *string) *string {
	if queue == nil {
		return initial
	}
...
// MinBy returns an element of P1StringQueue containing the minimum value, when compared to other elements
// using a passed func defining ‘less’. In the case of multiple items being equally minimal, the first such
// element is returned. Panics if there are no elements.
func (queue *P1StringQueue) MinBy(less func(*string, *string) bool) *string {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
// MaxBy returns an element of P1StringQueue containing the maximum value, when compared to other elements
// using a passed func defining ‘less’. In the case of multiple items being equally maximal, the first such
// element is returned. Panics if there are no elements.
func (queue *P1StringQueue) MaxBy(less func(*string, *string) bool) *string {
	queue.s.RLock()
	defer queue.s.RUnlock()
...
func (queue *P1StringQueue) Equals(other *P1StringQueue) bool {
	if queue == nil {
		if other == nil {
			return true
		}
...
func (queue *P1StringQueue) StringList() []string {
	if queue == nil {
		return nil
	}
...
func (queue *P1StringQueue) String() string {
	return queue.MkString3("[", ", ", "]")
}

// MkString concatenates the values as a string using a supplied separator. No enclosing marks are added.
func (queue *P1StringQueue) MkString(sep string) string {
	return queue.MkString3("", sep, "")
}

// MkString3 concatenates the values as a string, using the prefix, separator and suffix supplied.
func (queue *P1StringQueue) MkString3(before, between, after string) string {
	if queue == nil {
		return ""
	}
...
func (queue *P1StringQueue) UnmarshalJSON(b []byte) error {
	queue.s.Lock()
	defer queue.s.Unlock()
...
func (queue P1StringQueue) MarshalJSON() ([]byte, error) {
	queue.s.RLock()
	defer queue.s.RUnlock()
