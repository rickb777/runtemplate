...
// BuildX1IntSetFromChan constructs a new X1IntSet from a channel that supplies
...
func (set *X1IntSet) Fold(initial int, fn func(int, int) int) int {
	set.s.RLock()
	defer set.s.RUnlock()
...
func (set *X1IntSet) mkString3Bytes(before, between, after string) *strings.Builder {
	b := &strings.Builder{}
...
func (set *X1IntSet) GobDecode(b []byte) error {
	set.s.Lock()
	defer set.s.Unlock()
...
// You must register int with the 'gob' package before this method is used.
func (set X1IntSet) GobEncode() ([]byte, error) {
	set.s.RLock()
	defer set.s.RUnlock()
