...
func (mm *TX1ApplePearMap) slice() TX1ApplePearTuples {
	...
	s := make(TX1ApplePearTuples, 0, len(mm.m))
	for k, v := range mm.m {
...
func (mm *TX1ApplePearMap) MkString4(before, between, after, equals string) string {
	if mm == nil {
		return ""
	}
...
func (mm *TX1ApplePearMap) mkString4Bytes(before, between, after, equals string) *strings.Builder {
	b := &strings.Builder{}
...
// MkString4 concatenates the map key/values as a string, using the prefix, separator and suffix supplied.
func (ts TX1ApplePearTuples) MkString4(before, between, after, equals string) string {
	if ts == nil {
		return ""
	}
...
func (ts TX1ApplePearTuples) mkString4Bytes(before, between, after, equals string) *strings.Builder {
	b := &strings.Builder{}
	sep := before
	for _, t := range ts {
		b.WriteString(sep)
